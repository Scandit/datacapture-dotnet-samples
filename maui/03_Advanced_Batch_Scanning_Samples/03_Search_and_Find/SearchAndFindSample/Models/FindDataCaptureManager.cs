...
{
    private static readonly Lazy<FindDataCaptureManager> instance = new(
        () => new FindDataCaptureManager(), LazyThreadSafetyMode.PublicationOnly);

    public static FindDataCaptureManager Instance
    {
        get { return instance.Value; }
    }

    public BarcodeFind BarcodeFind { get; }
    public BarcodeFindSettings Settings { get; private set; } = null!;
    ...
    public void SetupForSymbology(Symbology symbology)
    {
        ...
        this.Settings = new BarcodeFindSettings();

        // We enable only the given symbology.
        this.Settings.EnableSymbology(symbology, true);

        // We apply the new settings to the barcode find.
        this.BarcodeFind.ApplySettingsAsync(this.Settings);
    }
    ...
    public void SetupSearchedItems(string data)
    {
        ...
        // one item, but for real case we can set several at once.
